...
def password_reset_token_created(sender, instance, reset_password_token, *args, **kwargs):
    # 👇 Deep link that opens Flutter app directly
    reset_link = f"codeleap://reset-password?token={reset_password_token.key}"
    
    # Also provide web fallback
    web_link = f"http://localhost:8000/reset-password?token={reset_password_token.key}"

    send_mail(
        subject="CodeLeap - Reset Your Password",
        message=f"""
Hi {reset_password_token.user.username},

You requested a password reset for your CodeLeap account.
